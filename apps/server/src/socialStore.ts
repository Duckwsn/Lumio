...
export class SocialStore {
  ...
  summary(house: HouseRecord, userId: string): HouseSummary {
    return { id: house.id, name: house.name, initials: initials(house.name), avatar: house.avatar, role: house.members.get(userId)?.role ?? "MEMBER", memberCount: house.members.size, onlineCount: [...house.members.values()].filter((m) => m.presence !== "OFFLINE").length, primaryRoomId: house.primaryRoomId };
  }
  ...
  createInvite(houseId: string, actor: User, input: { expiresInHours: 1 | 24 | 168; maxUses: number; role?: HouseRole }) {
    ...
    house.invites.set(invite.id, invite); this.inviteByToken.set(crypto.createHash("sha256").update(token).digest("hex"), invite); this.log(house, "INVITE_CREATED", `${actor.displayName} criou um convite.`, actor); return invite;
  }
  ...
  acceptInvite(token: string, user: User) {
    ...
    const invite = this.inviteByToken.get(crypto.createHash("sha256").update(token).digest("hex"))!; const house = this.houses.get(invite.houseId)!;
    if (!house.members.has(user.id)) { this.addMembership(house, user, invite.role); invite.uses += 1; }
