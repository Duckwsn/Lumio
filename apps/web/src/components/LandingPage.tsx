...
export function LandingPage({ navigate, googleOnlyPreview = false }: { navigate: Navigate; googleOnlyPreview?: boolean }) {
  useEffect(() => { document.title = "Lumio — fiquem juntos, mesmo de longe"; }, []);
  const signupPath = googleOnlyPreview ? "/login" : "/register";
  return <div className="landing-v3">
    <a className="skip-link" href="#landing-content">Ir para o conteúdo principal</a>
    <header className="landing-header"><div className="landing-container landing-header-inner"><Brand /><nav aria-label="Acesso"><button type="button" className="landing-header-login" onClick={() => navigate("/login")}>Entrar</button><button type="button" className="landing-button landing-button-small" onClick={() => navigate(signupPath)}>{googleOnlyPreview ? "Acessar prévia" : "Criar conta"} <ArrowRight size={15} /></button></nav></div></header>
    <main id="landing-content">
      <section className="landing-hero-v3 landing-container"><div className="landing-hero-copy"><span className="landing-overline"><i /> UM LUGAR PARA A SUA TURMA</span><h1>Fiquem juntos,<br /><em>mesmo de longe.</em></h1><p>Assista, ouça e converse com todo mundo no mesmo lugar. Uma Party só de vocês.</p><div className="landing-hero-actions"><button type="button" className="landing-button" onClick={() => navigate(signupPath)}>{googleOnlyPreview ? "Acessar prévia" : "Criar minha Casa"} <ArrowRight size={18} /></button><button type="button" className="landing-text-button" onClick={() => navigate("/login")}>Já tenho conta <ArrowRight size={16} /></button></div><div className="landing-hero-note"><span className="landing-note-rule" /> A noite fica melhor quando ninguém fica de fora.</div></div><PartyDemo /></section>
