...
export class DriveProvider implements MediaProviderAdapter {
  readonly id = "google-drive" as const; private state: PlaybackState = "idle"; private lastMediaId = ""; private loadGeneration = 0; private recoveryAttempted = false;
  ...
  private onError = () => {
    ...
    void this.ticket(this.lastMediaId).then((url) => {
      this.video.addEventListener("loadedmetadata", () => { this.seek(position); if (wasPlaying) void this.play().catch(() => undefined); }, { once: true });
      this.video.src = url; this.video.load();
    }).catch((error) => { this.state = "error"; this.onEvent({ type: "error", code: "PROVIDER_ERROR", message: error instanceof Error ? error.message : "Este vídeo não está disponível." }); });
  };
  private async ticket(mediaId: string) { const response = await fetch(`${this.apiUrl}/api/google-drive/files/${encodeURIComponent(mediaId)}/playback`, { method: "POST", credentials: "include", headers: { Authorization: `Bearer ${this.token}`, "Content-Type": "application/json" }, body: JSON.stringify({ roomId: this.roomId }) }); const data = await response.json() as { url?: string; message?: string }; if (!response.ok || !data.url?.startsWith("/api/google-drive/playback/")) throw new Error(data.message ?? "Este vídeo do Drive está indisponível."); return new URL(data.url, this.apiUrl).toString(); }
  async load(media: MediaState) { if (this.lastMediaId === media.mediaId && this.video.src) return; if (media.mimeType && !this.video.canPlayType(media.mimeType)) throw new Error("Este formato de vídeo não é compatível com seu navegador."); const generation = ++this.loadGeneration; this.state = "loading"; this.lastMediaId = media.mediaId; this.recoveryAttempted = false; const url = await this.ticket(media.mediaId); if (generation !== this.loadGeneration) return; this.video.src = url; this.video.load(); }
