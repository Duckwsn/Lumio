...
export class DriveProvider implements MediaProviderAdapter {
  ...
  private onError = () => {
    ...
    void this.ticket(this.lastMediaId, controller.signal).then((url) => {
      ...
      this.metadataHandler = () => { this.metadataHandler = null; this.seek(position); if (wasPlaying) void this.play().catch(() => undefined); };
  ...
  async load(media: MediaState) { if (this.lastMediaId === media.mediaId && this.video.src) return; if (media.mimeType && !this.video.canPlayType(media.mimeType)) throw new Error("Este formato de vídeo não é compatível com seu navegador."); const generation = ++this.loadGeneration; this.ticketRequest?.abort(); if (this.metadataHandler) this.video.removeEventListener("loadedmetadata", this.metadataHandler); this.metadataHandler = null; const controller = new AbortController(); this.ticketRequest = controller; this.state = "loading"; this.lastMediaId = media.mediaId; this.recoveryAttempted = false; const url = await this.ticket(media.mediaId, controller.signal); if (this.destroyed || generation !== this.loadGeneration) return; this.video.src = url; this.video.load(); }
  ...
  async sync(media: MediaState, options?: { force?: boolean }) { const changed = this.lastMediaId !== media.mediaId; await this.load(media); if (this.destroyed || this.lastMediaId !== media.mediaId) return; const target = expectedPosition(media); if (changed || Math.abs(target - this.getCurrentTime()) > (options?.force ? 1.25 : 2.5)) this.seek(target); if (this.video.playbackRate !== media.playbackRate) this.setPlaybackRate(media.playbackRate); if (media.state === "playing" && this.video.paused) await this.play(); if (["paused", "ended"].includes(media.state) && !this.video.paused) this.pause(); }
